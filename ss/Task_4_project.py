...
class YandexMusicApp(ctk.CTk):
    def __init__(self):
        ...
        self.info_label.grid(row=1, column=2, columnspan=2, padx=10, pady=10, sticky="nsew")
        ...
        self.time_slider.place(relx=0.01, rely=0.95, relwidth=0.98)
    ...
    def search_music(self):
        ...
        def search_thread():
            try:
                ...
                self.info_label.configure(text=f"❌ Ошибка поиска: {e}")
                self.progress.set(0)
            finally:
    ...
    def on_select(self, choice):
        try:
            ...
            self.info_label.configure(
                text=f"🎵 {self.selected_song.title}\n"
                     f"👤 {artists}\n"
                     f"💿 {album}\n"
                     f"⏱️ {self.format_time(duration_sec)}"
            )
        except Exception as e:
            self.info_label.configure(text=f"Ошибка выбора: {e}")
    ...
    def download_track(self):
        ...
        def download_thread():
            try:
                ...
                self.info_label.configure(text=f"✅ Скачано: {filename}")
                self.progress.set(1)
                self.load_downloaded_tracks()

            except Exception as e:
                self.info_label.configure(text=f"❌ Ошибка скачивания: {e}")
                self.progress.set(0)
            finally:
    ...
    def load_downloaded_tracks(self):
        ...
        if files:
            ...
            if self.current_track_index == -1:
                self.downloaded_combobox.set(files[0])
    ...
    def on_downloaded_select(self, choice):
        ...
        try:
            ...
            self.info_label.configure(text=f"Ошибка выбора файла: {e}")
    ...
    def play_selected_track(self):
        ...
        if not self.is_valid_mp3(filepath):
            self.info_label.configure(text=f"❌ Поврежденный файл:\n{self.downloaded_files[self.current_track_index]}")
            return
        ...
        try:
            ...
            self.info_label.configure(text=f"▶ Сейчас играет:\n{self.downloaded_files[self.current_track_index]}")

        except Exception as e:
            self.info_label.configure(text=f"Ошибка воспроизведения: {e}")
